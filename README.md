...
Deploy the frontend to Vercel and the backend as a persistent Node service on Render or Railway. Use MongoDB Atlas for persistence and configure the Google AI Studio/Gemini API key on the backend. Set the frontend `BACKEND_URL` to the deployed backend URL and set the backend `FRONTEND_ORIGIN` to the deployed frontend URL.
