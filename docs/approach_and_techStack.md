...
Use Groq through its OpenAI-compatible chat-completions API. The default model is `llama-3.3-70b-versatile`, selected for its free-tier availability and structured JSON response support. The implementation keeps the provider behind a `TextGenerator` interface so the model can be replaced without changing the pipeline.
...
The backend uses `LLM_API_KEY`, `LLM_MODEL`, and `LLM_TIMEOUT_MS`. The current default is Groq `llama-3.3-70b-versatile` through the OpenAI-compatible chat-completions API. Provider responses are retried with bounded backoff for transient failures, while malformed JSON and schema mismatches receive a bounded repair attempt.
