...
The frontend and backend must share `AUTH_INTERNAL_SECRET`. The backend must configure `FRONTEND_ORIGIN`, `MONGODB_URI`, and `LLM_API_KEY` for Gemini. A sleeping backend can interrupt generation, so a persistent service is preferred for the demo.